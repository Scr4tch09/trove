...
- **Images**: products with an `image` field (Amazon CDN URL from the product page) render a real photo; without one the card falls back to the icon-on-gradient placeholder. Image embedding from m.media-amazon.com is disclosed in `datenschutz.html`.
